<template>
  ...
  <div class="grid grid-cols-1 md:grid-cols-3 gap-5 p-8">
      <div v-for="hotel of searchedHotels" :key="hotel.id" class="bg-white shadow rounded-xl" >
        ...
        <div class="p-3">
          <h3 class="inline-block font-semibold border-b-2 border-slate-300 hover:border-purple-300">{{hotel.name}}</h3>
        <p class="mb-2">{{hotel.mapMarker.label}}</p>
        <router-link to="/" class="inline-block px-3 py-1 rounded border-2 border-purple-300 hover:bg-purple-300 hover:text-white transition-colors
